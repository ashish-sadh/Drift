...
- V6 visual evolution in 3 reversible elements (separate commits, separate reviews) is materially better than the cycle-869 monolithic theme overhaul shape. Multi-commit incremental UI ships should be the default; monolithic redesigns require justification, not the other way around.

### Review Cycle 10262 (2026-05-13)
- TestFlight reach is a *measurable* part of the product, not a tenet aspiration. Build 243's archive failure yesterday means the FM extraction breakthrough, GLP-1 data model, and activation banner are all invisible to testers — 24h of best-in-history velocity translates to zero user signal until the archive unblocks. Standing rule: any failed archive within the last 24h is a P0 senior task automatically (no manual triage gap). Filed as #770. **VALIDATED cycle 10888: rule fired correctly, 6 builds shipped in 72h.**
- A passive activation lever (#759 dashboard banner) is half a lever — it ships the surface but defers the *act of asking*. Pair every passive lever with one active ask (human DM in parallel). Cycle 10262 finally shipped the banner after 3 cycles of recommendation; the test of whether the lever works is *traffic*, not *deploy*. **STILL UNRESOLVED cycle 10888: 4 days since banner ship, zero DMs sent — the lesson hasn't sedimented as a rule yet. Filed #789 to make it structural.**
- Apple Foundation Models is the right platform bet to lean into. 4 surfaces migrated in 24h with @Generable schemas, behind a feature flag with eval-gated cutover. Platform-leverage moves like this justify "AI-first, privacy-first" — Apple did the model work, we integrate. Lean in more. **EXTENDED cycle 10888: `.foundationModels` chat backend wired (Task 1 PR A). Pattern is now templated across both extraction AND chat surfaces.**
