...
If you know me, you know how much I geek out about health metrics — whether you asked or not. **Drift** is the app I built because of it. A small language model runs on your phone, reads your data from Apple Health, and answers questions about it by chatting. No accounts. No subscription. No server to hold your data hostage — because I don't run one. It's on twenty-six phones now, friends and friends-of-friends, free for everyone. There are countless health apps on the App Store; the only thing new is that I could now build this one I actually liked — in my spare time, by myself. I hope you try it — the public beta is [on TestFlight](https://testflight.apple.com/join/NDxkRwRq).

But this post isn't really about Drift. The interesting part is what built it.

I didn't want to sit with Claude Code in a chat window for months, vibe-coding until something shipped. I wanted a simple `while true` loop to develop, test, and release the app on its own, with the kind of engineering discipline you'd expect from a real team. I've heard plenty of pitches about AI teams and AI engineers and multi-agent orchestras; I wanted the basics first. A loop that keeps going. A loop that builds its own harness.

So I started one. A bash `while true` around Claude Code — what Geoffrey Huntley calls a [Ralph loop](https://ghuntley.com/ralph/) — and spent my engineering time not on the app but on the scaffolding around the loop. Over months the scaffolding got sophisticated about its own engineering discipline. I seeded it with two personas — a Product Designer and a Principal Engineer — and across fifty-plus product reviews they grew into something that behaves like an experienced app developer and an experienced product manager. It learned from its mistakes, from my nudges, and from the feedback twenty-six beta users filed back. The releases are stable because the loop is simple and the harness around it is disciplined. (I try not to push every build — the Ralph loop produces a few a day, and my dogfooders would get tired.)

The thesis of this essay, after two weeks of watching it run mostly unattended: **human attention is the scarce resource, and the harness is where your taste actually lives.** Not in the model. Not in the prompt. In the scaffolding around the model — the gates that refuse bad tool calls, the reconciliation that doesn't trust session memory, the personas that compound taste, the dashboards I can read from my phone.
...
The scaffolding around that loop is made of five patterns, one per section below. Each came from a specific failure.
