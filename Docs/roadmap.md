...
- **Crash hunt + analytical breadth + UX refinement sprint (P0, cycle 9688 sprint)** — Active crash audit on recent backup/launch/calorie code (#687, tenet #8). Multi-intent splitting in chat — "log lunch and update weight" executes both tools sequentially (#688, carries #384). Smart meal reminders from user's median meal time (#690, carries #385). `cycle_biomarker_correlation` analytical tool — first cross-domain insight tying menstrual phase to biomarkers (#689). Food DB +30 South Indian regional (Kerala/Tamil/Andhra/Karnataka — #691, tenet #5). Goal milestone celebration (#692, weight roadmap). Hydration adherence dashboard insight card (#693, extends proactive triad to quad). Exercise muscle-group icons (#697, Boostcamp visual gap). UX refinement: tighten food diary row density (#696, tenet #7). Recurring AI quality: failing-queries.md refresh (#694), FoodLoggingGoldSet run (#695). State.md refresh to build 232 (#698). Build 232 published.
