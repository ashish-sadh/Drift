...
What's working: core tracking across all domains, AI chat foundation, 21 tools (5 analytical tools active), dual-model pipeline + cloud BYOK, ~2,252 tests (1,219 iOS + ~1,033 DriftCore, macOS 0.1s pure-logic), ~3,180 foods (deep Indian/Asian/European coverage, +30 Rajasthani/Gujarati/Andhra/German/Cambodian batch), state machine, structured chat cards, photo-attached meal logging, cross-session context, recent foods quick-log, GLP-1/medication tracking shipped (#580). Build 213 in project.yml (214 bumped — archive blocked by iOS 26.4 platform not installed in Xcode).

What's not: P0 TestFlight archive blocked (#614 — Xcode iOS 26.4 platform support not installed; requires Xcode > Settings > Platforms > iOS download), one open test failure (#568 testPortionScaling_DecimalServings), Settings → Feedback (#329 ten-cycle deferral → #617), planning crash (#407 → #619), TestFlight watchdog loop when archive broken (#626).
...
- **Reliability + delivery sprint (P0, cycle 8999 sprint)** — TestFlight gap is now 17 builds (204–220) dark; this cycle is dedicated to unblocking delivery and tightening the harness. INFRA: TestFlight hook suppress in human sessions (#640). Stop bumping CURRENT_PROJECT_VERSION while archive failed (#641). Heartbeat commit batching (#642). State.md pre-commit freshness hook (#643). State.md refresh to build 220 (#644). Surface TestFlight unblock in command-center (#645). Releases.json gap reconciliation (#646). AIChatView ViewModel extraction (#647). Verify proactive-card triad on dashboard (#648). Settings → Feedback (#617) is JUNIOR P0 — 12-cycle deferral, no exceptions.
