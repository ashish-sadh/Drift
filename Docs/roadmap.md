...
- **AI Chat depth — eval closure & analytical expansion (P0, cycle 7564 sprint)** — FoodLoggingGoldSetTests run cycle 7484 + fix failures (#468). LLM prompt quality audit cycle 7484 (#469). FoodLoggingGoldSet +12 — verify 4 fixed failing-query categories (#470). MultiTurnRegression +8 macro goal + micronutrient chains (#471). State.md refresh to build 176 (#472). failing-queries.md refresh cycle 7462→7484 (#473). Food DB +30 West African cuisine (#474). Food DB +30 Japanese home cooking expansion (#475). P0 bug: combo delete option (#465). Still open: supplement_insight (#417), food_timing_insight (#418), planning crash fix (#407), USDA Phase 2 (#345).
