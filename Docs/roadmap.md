...
What's working: core tracking across all domains, AI chat foundation, 20 tools, dual-model pipeline, ~2,900 tests (2,061 iOS + ~850 DriftCore, macOS 0.1s pure-logic), 2,511+ foods (USDA batch +500), state machine, structured chat cards, build 182 on TestFlight.

What's not: supplement_insight + food_timing_insight (2 analytical tools crashed twice each), photo logging recovery UX still scan-heavy, users still compute units/portions manually, remote model pipeline not wired yet.
