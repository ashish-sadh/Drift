...
- **AI Chat depth — Photo Log hardening & telemetry-driven reliability (P0, cycle 3985 sprint)** — Photo Log provider fallback chain (Anthropic→OpenAI→Gemini on transient fails, #300). `/debug last-failures` chat command exposes persisted telemetry for on-device debugging (#301). IntentClassifier per-domain confidence thresholds calibrated from telemetry (#302). PhotoLogTool eval +12 provider-failure cases (#303). Gold-set +15 branded food variants — Chobani/Quest/Kodiak (#304). Multi-turn regression +10 photo-log follow-up corrections (#306). Per-stage elapsed indicator in chat typing UX (#309). Food DB +60 (Caribbean #305, breakfast cereals/oatmeal brands #308). Housekeeping: failing-queries.md refresh cycle 3022→3985 (#307).
