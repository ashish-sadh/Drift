...
- **AI Chat depth — analytical tool engines + eval quality + food DB depth (P0, cycle 8581 sprint)** — supplement_insight analytics engine (#565, SupplementService.adherenceStats + response card). food_timing_insight analytics engine (#566, FoodEntry meal timing aggregation + response). State.md refresh to build 197 (#567). Fix testPortionScaling_DecimalServings (#568). Fix testRouterPrompt_TokenCeiling (#569). failing-queries.md refresh (#570). FoodLoggingGoldSet run (#571). Indian branded protein foods +30 (#572). UX: recent foods quick-log (#573). GLP-1 design doc filed (#574).
