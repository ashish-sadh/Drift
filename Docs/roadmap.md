...
- **AI Chat depth — analytical tools, eval coverage & USDA batch import (P0, cycle 7689 sprint)** — supplement_insight analytical tool with 5+ eval cases (#417/#488). food_timing_insight analytical tool with 5+ eval cases (#418/#488). FoodLoggingGoldSetTests run cycle 7689 + fix failures (#481). LLM prompt quality audit cycle 7689 (#483). FoodLoggingGoldSet +8 tie-break + weight_trend eval cases (#484). USDA batch import top 500 USDA foods → foods.json (#485). State.md refresh to build 178 (#482). failing-queries.md refresh cycle 7484→7689 (#486). USDA DEMO_KEY → registered API key (#487). Planning crash fix — planning-service.sh exit path (#407).
