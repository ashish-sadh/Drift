...
What's working: core tracking across all domains, AI chat foundation, 21 tools (5 analytical tools active), dual-model pipeline + cloud BYOK, ~2,252 tests (1,219 iOS + ~1,033 DriftCore, macOS 0.1s pure-logic), ~3,150 foods (deep Indian/Asian/European coverage), state machine, structured chat cards, photo-attached meal logging, cross-session context, recent foods quick-log, build 200 on TestFlight.

What's not: GLP-1/medication tracking (design doc #574 filed, implementation tasked as #580 — MFP launched free GLP-1 April 28), two open test failures (#568, #569), Settings → Feedback (#329 nine-cycle deferral), planning crash (#407).
...
- **AI Chat depth — GLP-1 depth, eval quality & test hygiene (P0, cycle 8789 sprint)** — Fix testPortionScaling_DecimalServings (#587). Settings → Feedback row — close 10-cycle deferral (#588). Planning crash fix (#589). Eval cases for log_medication (#590). State.md refresh to build 203 (#591). GLP-1 dose reminder notifications (#592). FoodLoggingGoldSet run cycle 8789 (#593). failing-queries.md refresh cycle 8666→8789 (#594). LLM prompt quality audit cycle 8789 (#595). Food DB +30 Indian protein staples (#596). Per-stage elapsed indicator (#597). MultiTurnRegression +8 GLP-1 chains (#598).
